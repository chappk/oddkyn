...
export class BoardEditor extends Phaser.Polygon
{
    ...
    game: Oddkyn.Game;
    // Size of the BoardEditor
    size: number
    // Level 0 of the 
    emptyBoard: Array<Array<CaseEditor>>;
    // Hold groups to display the board
    boardLayer: Array<Phaser.Group>;
    // Hold groups to display characters
    charactersLayer: Array<Phaser.Group>;
    // Group which represents the board
    board: Phaser.Group;
    ...
    constructor(game: Oddkyn.Game, size = 17)
    {
        ...
        this.game = game;
        this.size = size;
        this.emptyBoard = new Array<Array<CaseEditor>>();
        this.board = game.add.group();
        this.boardLayer = new Array<Phaser.Group>();
        this.charactersLayer = new Array<Phaser.Group>();
        this.addLayer();
        for(var i: number = 0; i < this.size; i++) 
        {
            this.emptyBoard[i] = new Array<CaseEditor>();
            for(var j: number = 0; j < this.size; j++) 
            {
                let [x, y] = this.ijToxy(i, j);
                this.emptyBoard[i][j] = new CaseEditor(game, this, x, y, "empty");
                this.boardLayer[0].add(this.emptyBoard[i][j]);
            }
        }
    ...
    public addLayer()
    {
        this.boardLayer.push(this.game.add.group(this.board));
    ...
    private click(pointer: Phaser.Pointer)
    {
        ...
        if(this.contains(pointer.x, pointer.y))
        {
            ...
            if(i < this.size && j < this.size && i >= 0 && j >= 0 && 
                this.emptyBoard[i][j] == null)
            {
                this.addCase(i, j);
            } 
            else
            {
                let c =this.emptyBoard[i][j].stack("grass");
    ...
    public stack(c: CaseEditor, key: string, ori: Orientation)
    {
        ...
        if (cTop.floor >= this.boardLayer.length)
        {
            this.addLayer();
        }
        this.boardLayer[cTop.floor].add(cTop);
        this.boardLayer[cTop.floor].sort('y', Phaser.Group.SORT_ASCENDING);
        return cTop;
    }
